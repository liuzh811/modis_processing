...
#define a function to filter the data
#m is the data quality raster
#return a raster stack, layer 1-7 represent data quality for band 1 - 7
QAfind.mt = function(m){ 
  ext = extent(m)
  proj.geo = projection(m)
 ...
 #change back to raster
  q0 = raster(matrix(qc[,1], nrow = nr, ncol = nc, byrow = TRUE),    
                xmn=ext@xmin, xmx=ext@xmax,
                ymn=ext@ymin, ymx=ext@ymax, 
                crs=CRS(proj.geo))
  
  q1 = raster(matrix(qc[,2], nrow = nr, ncol = nc, byrow = TRUE),    
              xmn=ext@xmin, xmx=ext@xmax,
              ymn=ext@ymin, ymx=ext@ymax, 
              crs=CRS(proj.geo))
  
  q2 = raster(matrix(qc[,3], nrow = nr, ncol = nc, byrow = TRUE),    
              xmn=ext@xmin, xmx=ext@xmax,
              ymn=ext@ymin, ymx=ext@ymax, 
              crs=CRS(proj.geo))
  
  q3 = raster(matrix(qc[,4], nrow = nr, ncol = nc, byrow = TRUE),    
              xmn=ext@xmin, xmx=ext@xmax,
              ymn=ext@ymin, ymx=ext@ymax, 
              crs=CRS(proj.geo))
  
  q4 = raster(matrix(qc[,5], nrow = nr, ncol = nc, byrow = TRUE),    
              xmn=ext@xmin, xmx=ext@xmax,
              ymn=ext@ymin, ymx=ext@ymax, 
              crs=CRS(proj.geo))
  
  q5 = raster(matrix(qc[,6], nrow = nr, ncol = nc, byrow = TRUE),    
              xmn=ext@xmin, xmx=ext@xmax,
              ymn=ext@ymin, ymx=ext@ymax, 
              crs=CRS(proj.geo))
              
  q6 = raster(matrix(qc[,7], nrow = nr, ncol = nc, byrow = TRUE),    
              xmn=ext@xmin, xmx=ext@xmax,
              ymn=ext@ymin, ymx=ext@ymax, 
              crs=CRS(proj.geo))
  
  return(stack(q0,q1,q2,q3,q4,q5,q6))
}
